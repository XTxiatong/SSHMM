...
@author: xiatong
This is the pyspark code for WWW2019
"""
...
def calcAlpha(x,begin):
	...
	#P(O|^)	
	po = sum(alpha_r[N-1,:])
	return (input,(X_r,alpha_r,beta_r,gamma_r,xi_r,A_r,pi_r),po)
...
def calcBeta(x,begin):
	...
	return (input,(X_r,alpha_r,beta_r,gamma_r,xi_r,A_r,pi_r),(po,po_a))
...
def calcGamma(x,begin):
	...
	return(input,(X_r,alpha_r,beta_r,gamma_r,xi_r,A_r,pi_r))
...
def updateMuSigma(gamma_all,begin):
	global X,D,N,M,K,R,L
	global mu,sigma
	for k in range (K):
		Gamma_k = sum(gamma_all[:,:,k])
		if Gamma_k == 0:
			print('Gamma_k=0!')
			pass
		else:
			for l in range(0,L):
				Gamma_rn = gamma_all[:,:,k]
				X_rn = X[:,begin:begin+N,l]
...
if __name__ == "__main__":
	...
	O = np.load(root) #pre-precessed data

	#init
	#Fixed parameters.
	N = 12 # the length of sequence for each iter(if the sequence is too long, then it's hard to train)
	M = N #The decode length each time
	K=int(in1) #The number of latent states.
	X=O[:,:2*24,:] #shape[R,N*n,L] the observation sequences for train, e.g., the first two day
	D=O[:,:2*24,:] #shape[R,N2,L] the observation sequences for decode, e.g., the first two day
	...
	sigma=np.ones([K,L])*0.01 #fix 0.01
	...
	output='result/result_R'+str(D.shape[0])+'_'+str(D.shape[1])+'_N'+str(N)+'_S'+str(K)+'max_iter'+str(maxIter)+'_pi_sigma/'
	print(output)
	...
	for i in range(max_epoch): # run max_epoch with different initialization
		for j in range (maxIter):
			for n in range(0,X.shape[1],N):  # n is the beginning time slot
				if ( n + N - 1 < X.shape[1]): #the end is less than the last time slot
					print('epoch:',i,'iter:',j,',begin:',n,',end:',n+N-1,'mu:', np.sum(mu))
					...
					Gamma = AllRDD.map(lambda x: (1,(x[0],x[1][3]))).groupByKey().map(lambda x: extraGamma(x))
					musigma = Gamma.map(lambda x:updateMuSigma(x,n))
					mu_rdd = musigma.map(lambda x:x[0]).collect()
					sigma_rdd = musigma.map(lambda x:x[1]).collect()
					mu = mu_rdd[0]
					sigma = sigma_rdd[0]
					
			decodeRDD = AllRDD.join(DRDD)
			state = decodeRDD.map(lambda x: viterbi(x)).map(lambda x:(1,x)).groupByKey().map(lambda x: extraSate(x)).collect()
			Pi = AllRDD.map(lambda x: (1,(x[0],x[1][6]))).groupByKey().map(lambda x: extraPi(x)).collect()
			A = AllRDD.map(lambda x: (1,(x[0],x[1][5]))).groupByKey().map(lambda x: extraA(x)).collect()
			savetxt(output+'mu%d'%i+'_iter%d'%j,mu,fmt='%.4f')
			savetxt(output+'sigma%d'%i+'_iter%d'%j,sigma,fmt='%.4f')
			savetxt(output+'pi%d'%i+'_iter%d'%j,Pi[0],fmt='%.4f')
			save(output+'A%d'%i+'_iter%d'%j,A[0])
