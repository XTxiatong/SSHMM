...
def calcForward(r,X):
	...
	for k in range(K):
		o=1
		for l in range (L):
			o=o*normfun(X[0][l],mu[r][k][l],sigma[r][k][l])
		alpha[0][k]=Pi[r][k]*o
	#n=1,2,...
	for n in range (1,len(X)):
		for k in range (K):
			o=1
			for l in range (L):
				o=o*normfun(X[n][l],mu[r][k][l],sigma[r][k][l])
	...
	for n in range (W-2,-1,-1):
		for k in range (K):
			...
			for j in range (K):
				o=1
				for l in range (L):
					o=o*normfun(X[n+1][l],mu[r][j][l],sigma[r][j][l])
...
def pre_state(r,X):
	...
	for n in range (T,N):
		...
		prediction[0,n-T,:] = mu[r,nextstate,:]
	return (r,prediction)
...
if __name__ == "__main__":
	...
	O = load(root)
	O = O[:,:,:]
	A=load('result_simpleHMM/result_R665_504_N24_S100max_iter5_pi_sigma/A0_iter0.npy')
	Pi=loadtxt('result_simpleHMM/result_R665_504_N24_S100max_iter5_pi_sigma/pi0_iter0')
	mu=load('result_simpleHMM/result_R665_504_N24_S100max_iter5_pi_sigma/mu0_iter0.npy')
	sigma=load('result_simpleHMM/result_R665_504_N24_S100max_iter5_pi_sigma/sigma0_iter0.npy')
	...
	sio.savemat('prediction_HMM_R665_504_N24_S100max_iter5_pi_sigma_iter0.mat',{'prediction':prediction}) 
